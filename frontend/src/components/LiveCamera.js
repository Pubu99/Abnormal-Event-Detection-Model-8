...
function LiveCamera({ onDetection }) {
  ...
  const handlePrediction = (message) => {
    const { data, timestamp, frame_number } = message;

    // Update current result with enhanced data INCLUDING FUSION
    setCurrentResult({
      ...data,
      ...
      fusion: data.fusion || null, // NEW: Fusion reasoning
    });

    // NEW: Update fusion reasoning display
    ...
    <div className="space-y-4">
      ...
      >
        ...
        {currentResult && isStreaming && (
          <div className="absolute bottom-4 left-4 right-4 bg-black bg-opacity-90 text-white p-4 rounded-lg shadow-lg">
              ...
              <div className="text-right">
                ...
                <p
                  className={`text-lg font-bold ${
                    currentResult.threat_level === "CRITICAL"
                      ? "text-red-500"
                      : currentResult.threat_level === "HIGH"
                      ? "text-orange-500"
                      : currentResult.threat_level === "MEDIUM"
                      ? "text-yellow-500"
                      : currentResult.threat_level === "LOW"
                      ? "text-blue-500"
                      : "text-green-500"
                  }`}
                >
                  {currentResult.is_dangerous ? "🚨" : "✓"}{" "}
                  {currentResult.threat_level || "INFO"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-700 grid grid-cols-4 gap-2 text-xs">
              ...
              <div>
                ...
                >
                  {currentResult.motion?.is_unusual ? "⚠ Unusual" : "✓ Normal"}
                </p>
              </div>
              ...
              <div>
                ...
                >
                  {currentResult.pose?.is_anomalous ? "⚠ Anomaly" : "✓ Normal"}
                </p>
              </div>
            ...
            {currentResult.alerts && currentResult.alerts.length > 0 && (
              <div className="mt-3 pt-3 border-t border-gray-700">
                ...
                {currentResult.alerts.slice(0, 2).map((alert, idx) => (
                  <div
                    key={idx}
